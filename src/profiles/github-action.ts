...
export const githubActionProfile: PipelineProfile = {
  name: 'github-action',
  ...
  applyEnv(env: NodeJS.ProcessEnv): void {
    ...
    set(
      'BUILD_URL',
      repo && process.env.GITHUB_RUN_ID ? `${serverUrl}/${repo}/actions/runs/${process.env.GITHUB_RUN_ID}` : undefined
    )
...
export default githubActionProfile
