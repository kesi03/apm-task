...
function main(): void {
  try {
    ...
    if (result.error) {
      tl.setResult(tl.TaskResult.SucceededWithIssues, `CI APM trace failed: ${result.error.message}`)
      return
    }
    if (result.status === 0) {
      tl.setResult(tl.TaskResult.Succeeded, 'CI APM trace sent')
    } else {
      tl.setResult(tl.TaskResult.SucceededWithIssues, `CI APM trace failed with exit code ${result.status}`)
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    tl.setResult(tl.TaskResult.SucceededWithIssues, `CI APM trace failed: ${message}`)
  }
}
